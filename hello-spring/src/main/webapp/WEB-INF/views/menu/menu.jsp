...
const url = menuRestOrigin + menuRestContextPath;
		...
		<div id="menu-container" class="text-center">
	        ...
	        -origin : protocol + host + port 
	                  http://localhost:9090   --->http://localhost:10000, https://localhost:9090 한글자만 달라도 동일근원이 아님 
		    - 비동기요청시 현재페이지 origin과 동일 origin으로만 요청할 수 있게 제한함.

			CORS Policy Cross Origin Resource Sharing 타오리진자원공유? 다른오리진과 자원을 공유한다.
			- 조건: 응답header에 Access-Control-Allow-Origin : 나의 origin값이 설정되어 있을것. 
	        

	        
	        Access to XMLHttpRequest at 'http://localhost:10000/springboot/menus' ->여기에 비동기요청을 하고 있었음.
	        from origin 'http://localhost:9090' ->여기로부터
	        has been blocked by CORS policy: ->CORS에 의해 차단되었다.
	        No 'Access-Control-Allow-Origin' header is present on the requested resource. ->요청한 자원에 이런 헤더가 존재하지 않는다.
	        */
...
/* 	const frmData = new FormData(e.target);
	...
	$.ajax({
		...
		complete(){
			$frm[0].reset();
			$(menuSearchFrm)[0].reset(); //jquery-[0]부터는 javascript객체 리셋
		...
		$.ajax({
			...
			success(data){		    //메소드를 만듬
				...
				error(xhr, statusText, err){
					const {status} = xhr;
					switch(status){
					case 404:alert("조회한 메뉴는 존재하지 않습니다."); break;
